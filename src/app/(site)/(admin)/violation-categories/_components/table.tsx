...
import { ViolationCategory } from "./form";
import { deleteViolationCategoryById, getAllViolationCategories } from "@/services/violation-categories";
import { usePagination } from "@/hooks/usePagination";

type ViolationCategoriesTableProps = {
  data: ViolationCategory[];
  setCategories: (categories: ViolationCategory[]) => void;
  handlePageChange: (page: number) => void;
  rootPath: string;
  minWidth: number;
};

const ViolationCategoriesPagination = ({
  meta,
  handlePageChange,
}: {
  meta: Meta;
...
export const ViolationCategoriesTable = ({
  data,
  setCategories,
  handlePageChange,
  rootPath,
  minWidth,
}: ViolationCategoriesTableProps) => {
  ...
  const deleteHandler = async (id: string) => {
    toast.loading("Loading...", { id: "deleteCategory" });
    try {
      const response = await deleteViolationCategoryById(id);
  ...
  const handleSearch = async (value: string) => {
    toast.loading("Mencari data...", { id: "getCategoriesBySearch" });
    try {
      const response = await getAllViolationCategories(1, value);
      toast.dismiss("getCategoriesBySearch");
      ...
      <Table className={`min-w-[${minWidth}px] shadow-md relative bg-white dark:bg-neutral-800`}>
        <TableHeader className="sticky shadow -top-[1px] bg-gray-100 dark:bg-neutral-700">
          <TableRow className="uppercase text-gray-900 dark:text-gray-100">
            ...
            <TableHead className="font-semibold text-gray-900 dark:text-gray-100">Nama Kategori</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {data.length === 0 ? (
